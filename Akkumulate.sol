...
/**
 *  AKKUMULATE - https://www.akkumulate.com https://t.me/akkumulate
 *  
 *  The first automated trading competition token on KCC, paying a new winner every hour of every day
 *  
 *  Each Tx is taxed 10% with 5% of the fees going to a winner's prize fund.
 *  The wallet with the highest trading volume in a given hour, is paid the prize fund, automatically in KCS.
 *  5% of each Tx is automatically added to the liquidity pool, putting positive pressure on the price
 *  
 *  50% burned, 5% in marketing wallet. Remaining 45% in LP for fair launch.
 */
...
contract Akkumulate is Context, IKRC20, Ownable {
    ...
    TradingStats private _currentLeader;
    ...
    string private constant _name = "Akkumulate";
    string private constant _symbol = "AKKU";
    ...
    uint256 private _competitionLength = 3600;
    ...
    event NewCompetitionStarted (uint256 _competitionEndTime);
    event AddressBlacklistModified (address indexed account, bool blacklisted);
    ...
    constructor (address platformWallet) {
        ...
        IKuswapRouter02 kuswapV2Router = IKuswapRouter02 (0xA58350d6dEE8441aa42754346860E3545cc83cdA);
    ...
    // We may also need to do this if AKKU will be used to deposit in farms (ref. Cerberus etc.)
    ...
    function setAddressBlacklisting (address account, bool blacklisted) external {
        ...
        require (account != _burnAddress, "Cannot blacklist the burn address");
        require (account != _kuswapV2Pair, "Cannot blacklist the pair address");
        require (account != _kuswapV2Router.factory(), "Cannot blacklist the factory address");
        require (account != owner(), "Cannot blacklist the contract owner");
        require (account != _feeSetter, "Cannot blacklist the feeSetter");
        _tradingStats[account].blacklist = blacklisted;
        emit AddressBlacklistModified (account, blacklisted);
    }
    
    // Help users who accidentally send tokens to the contract address
    // Does not affect the proper running of the contract - AKKU is specifically prevented from being withdrawn in this way
    function withdrawOtherTokens (address _token) external nonReentrant {
        require (_msgSender() == _feeSetter, "Only accessible by feeSetter");
        require (_token != address(this), "Can't withdraw AKKU from contract");
    ...
    function withdrawExcessKCS() external nonReentrant {
        require (_msgSender() == _feeSetter, "Only accessible by feeSetter");
        uint256 contractKCSBalance = address(this).balance;
        
        if (contractKCSBalance > 0)
            payable(_feeSetter).sendValue(contractKCSBalance);
    }
    ...
    function _getTransferAmount (uint256 amount) private returns (uint256) {
        if (!_enableFees) {
            ...
            _balance[address(this)] = _balance[address(this)].add(fee);
    ...
    function _transfer (address from, address to, uint256 amount) private {
        ...
        // Check we're not already adding liquidity and don't take fees if sender is the Kuswap pair (i.e. someone is buying AKKU).
        // Also do this if the competition end time has passed so we can pay the winner
    ...
    function _swapTokensForKCS (uint256 tokenAmount) private returns (uint256) {
        ...
        // Generate the kuswap pair path of AKKU -> WKCS
